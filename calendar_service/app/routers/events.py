...
from app.schemas.responses import MessageEventDelete

router = APIRouter()


@router.post("/", response_model=EventOut, summary="Создание события")
async def create_event(event_data: EventCreate, user: CurrentUser, calendar_service: EventServiceDeps):
...
async def get_event(event_id: int, user: CurrentUser, calendar_service: EventServiceDeps):
...
async def update_event(event_id: int, event_data: EventUpdate, user: CurrentUser, calendar_service: EventServiceDeps):
    event = await calendar_service.update_event(event_id, event_data, user)
    return event


@router.delete("/{event_id}", response_model=MessageEventDelete, summary="Удаление события")
async def delete_event(event_id: int, user: CurrentUser, calendar_service: EventServiceDeps):
    await calendar_service.delete_event(event_id, user)
    return MessageEventDelete()
