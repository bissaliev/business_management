...
async def org_structure_service(session: Annotated[AsyncSession, Depends(get_session)]) -> OrgStructureService:
...
async def division_service(session: Annotated[AsyncSession, Depends(get_session)]) -> DivisionService:
...
async def department_service(session: Annotated[AsyncSession, Depends(get_session)]) -> DepartmentService:
...
async def employee_service(session: Annotated[AsyncSession, Depends(get_session)]) -> EmployeeService:
...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)], user_client: AuthClientDeps) -> User:
    """Проверяет токен через User Service, возвращает данные (id, email, is_active, status, team_id, team_role)."""
...
async def check_admin_team_access(admin: Admin, team_id: int) -> None:
    """Пользователь должен быть администратором данной команды"""
...
def department_repo(session: Annotated[AsyncSession, Depends(get_session)]) -> DepartmentRepository:
...
async def check_admin_department_access(
    ...
    """Пользователь должен быть администратором команды в которой находится департамент"""
