...
class CommentService:
    ...
    async def create_comment(self, task_id: int, user: User, comment_data: CommentCreate) -> Comment:
        ...
        logger.info(f"Создан комментарий {comment.id} к задаче {task_id}")
        return comment
    ...
    async def update_comment(self, task_id: int, comment_id: int, user: User, comment_data: CommentCreate) -> Comment:
        ...
        comment = await self.repo.update(comment_id, **comment_data.model_dump(exclude_unset=True))
        await self.session.commit()
    ...
    async def _get_task_or_404(self, task_id) -> Task:
    ...
    async def _get_comment_or_404(self, comment_id) -> Comment:
    ...
    async def _require_author(self, comment_id: int, user_id: int) -> Comment:
